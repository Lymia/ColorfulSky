...
let add_gen_ore = function(block, parent_stone, cluster_size, cluster_count, min_y, max_y, biomes) {
    ...
    for (var i = 0; i < cluster_count; i++) { // Workaround for a horrible bug.
        ore_data_list.push({
            "block": block,
...
onEvent('worldgen.add', e => {
    if (!is_scrambled) {
        ...
        var rng = 2856674
        for (var i = 0; i < ore_data_list.length; i++) {
            var j = i + ((rng >> 20) % (ore_data_list.length - i))
            if (i != j) {
                var tmp = ore_data_list[i]
    ...
    for (var idx in ore_data_list) {
        var data = ore_data_list[idx]
        e.addOre(ore => {
